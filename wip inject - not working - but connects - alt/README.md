...
```bash
# Run the local browser-based Receive Items helper:
python local_receive_items.py --attach-on-start
...
### Local Receive Items Helper

The new local helper serves a simple browser UI on `http://127.0.0.1:27015/`.

Features in the first pass:

- local-only HTTP server
- local browser page for pasting `@U...` serials
- single inject
- batch inject
- live attach/status polling

This is intended as the first step toward a BL3-style "Receive Items" workflow
for BL4 without depending on the public site.
